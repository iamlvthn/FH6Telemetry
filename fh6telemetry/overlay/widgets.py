...
# Base cell geometry; wide panels are multiples of these.
CELL_W = 132
CELL_H = 104
SPACING = 6


def _wide(columns: int) -> int:
    return CELL_W * columns + SPACING * (columns - 1)


class BasePanel(QWidget):
    """Common background/styling and state plumbing for every panel."""

    def __init__(self, width: int = CELL_W, height: int = CELL_H) -> None:
        super().__init__()
...
class CorePanel(BasePanel):
    ...
    def paintEvent(self, _event) -> None:
        ...
        painter.setFont(theme.font(40, bold=True))
        painter.setPen(theme.SHIFT_CRIT if s.should_shift and self.blink else theme.TEXT)
        painter.drawText(QRectF(12, 28, 80, 64), Qt.AlignmentFlag.AlignCenter, gear_text)
        self._label(painter, "GEAR", 12, 90)
        ...
        painter.drawText(QRectF(95, 30, w - 110, 50), Qt.AlignmentFlag.AlignRight, f"{speed:0.0f}")
        painter.setFont(theme.font(9, mono=False))
        painter.setPen(theme.TEXT_DIM)
        painter.drawText(QRectF(95, 78, w - 107, 16), Qt.AlignmentFlag.AlignRight, unit)

        # RPM readout (small, right under bar).
        painter.setFont(theme.font(7, mono=False))
        painter.setPen(theme.TEXT_DIM)
        painter.drawText(
            QRectF(95, 28, w - 107, 12),
...
class InputsPanel(BasePanel):
    ...
    def paintEvent(self, _event) -> None:
        ...
        track = QRectF(12, self.height() - 9, self.width() - 24, 4)
        painter.setBrush(QColor(40, 44, 52, 200))
...
class PowerPanel(BasePanel):
    ...
    def paintEvent(self, _event) -> None:
        ...
        painter.setFont(theme.font(20, bold=True))
        painter.setPen(theme.TEXT)
        painter.drawText(QRectF(10, 18, self.width() - 20, 26), Qt.AlignmentFlag.AlignLeft, f"{s.power_hp:0.0f}")
        painter.setFont(theme.font(8, mono=False))
        painter.setPen(theme.TEXT_DIM)
        painter.drawText(QRectF(10, 26, self.width() - 14, 14), Qt.AlignmentFlag.AlignRight, "hp")

        painter.setFont(theme.font(8, mono=False))
        rows = [
            ("Torque", f"{s.torque_nm:0.0f} Nm"),
            ("Boost", f"{s.boost_psi:0.1f} psi"),
            ("Drive", s.drivetrain),
        ]
        y = 46
        for label, value in rows:
            painter.setPen(theme.TEXT_DIM)
            painter.drawText(QRectF(10, y, 60, 14), Qt.AlignmentFlag.AlignLeft, label)
            painter.setPen(theme.TEXT)
            painter.drawText(QRectF(self.width() - 80, y, 70, 14), Qt.AlignmentFlag.AlignRight, value)
            y += 14

        # Fuel bar.
        track = QRectF(10, self.height() - 12, self.width() - 20, 5)
        painter.setPen(Qt.PenStyle.NoPen)
        painter.setBrush(QColor(40, 44, 52, 200))
        painter.drawRoundedRect(track, 2, 2)
        fill = QRectF(track)
...
class GForcePanel(BasePanel):
    ...
    def paintEvent(self, _event) -> None:
        ...
        painter.setFont(theme.font(8, mono=False))
        painter.setPen(theme.TEXT)
        painter.drawText(QRectF(6, self.height() - 16, self.width() - 12, 12),
...
class HandlingPanel(BasePanel):
    ...
    def paintEvent(self, _event) -> None:
        ...
        painter.drawText(QRectF(10, 9, 80, 14), Qt.AlignmentFlag.AlignLeft, "UNDERSTEER")
        painter.setPen(theme.OVERSTEER)
        painter.drawText(QRectF(self.width() - 90, 9, 80, 14), Qt.AlignmentFlag.AlignRight, "OVERSTEER")

        track = QRectF(95, 13, self.width() - 190, 8)
        painter.setPen(Qt.PenStyle.NoPen)
        painter.setBrush(QColor(40, 44, 52, 200))
...
class LapsPanel(BasePanel):
    ...
    def paintEvent(self, _event) -> None:
        ...
        for label, value in rows:
            painter.setPen(theme.TEXT_DIM)
            painter.drawText(QRectF(10, y, 38, 14), Qt.AlignmentFlag.AlignLeft, label)
            painter.setPen(theme.TEXT)
            painter.drawText(QRectF(40, y, self.width() - 50, 14), Qt.AlignmentFlag.AlignRight, value)
            y += 15
        ...
        if s.delta_valid:
            ...
            text = "--.---"
        painter.setFont(theme.font(13, bold=True))
        painter.setPen(color)
        painter.drawText(QRectF(10, self.height() - 24, self.width() - 20, 20),
                         Qt.AlignmentFlag.AlignRight, text)
        painter.setFont(theme.font(7, mono=False))
        painter.setPen(theme.TEXT_DIM)
        painter.drawText(QRectF(10, self.height() - 22, 60, 14), Qt.AlignmentFlag.AlignLeft, "DELTA")
...
class PerformancePanel(BasePanel):
    ...
    def paintEvent(self, _event) -> None:
        s = self._state
        painter = self._begin()
        self._label(painter, "PERFORMANCE", 10, 6)

        top = s.top_speed_kph if self.use_metric else s.top_speed_kph * 0.621371
        unit = "km/h" if self.use_metric else "mph"
        painter.setFont(theme.font(9))
        painter.setPen(theme.TEXT_DIM)
        painter.drawText(QRectF(10, 20, 70, 14), Qt.AlignmentFlag.AlignLeft, "Top spd")
        painter.setPen(theme.TEXT)
        painter.drawText(QRectF(70, 20, self.width() - 80, 14), Qt.AlignmentFlag.AlignRight, f"{top:0.0f} {unit}")

        painter.setPen(theme.TEXT_DIM)
        painter.drawText(QRectF(10, 35, 70, 14), Qt.AlignmentFlag.AlignLeft, "Max pwr")
        painter.setPen(theme.TEXT)
        painter.drawText(QRectF(70, 35, self.width() - 80, 14), Qt.AlignmentFlag.AlignRight, f"{s.max_power_hp:0.0f} hp")

        # Acceleration runs as a small two-column grid.
        runs = list(s.accel_runs)
        painter.setFont(theme.font(8))
        x = 10
        y = 54
        col = 0
        for label, seconds in runs[:6]:
            painter.setPen(theme.TEXT_DIM)
            painter.drawText(QRectF(x, y, 60, 13), Qt.AlignmentFlag.AlignLeft, label)
            painter.setPen(theme.TEXT)
            painter.drawText(QRectF(x, y, 118, 13), Qt.AlignmentFlag.AlignRight, f"{seconds:0.2f}s")
            col += 1
            if col == 2:
                col = 0
                x = 10
        ...
        if s.active_run_label:
            ...
            painter.drawText(QRectF(10, self.height() - 14, self.width() - 20, 12),
                             Qt.AlignmentFlag.AlignLeft, f"running {s.active_run_label}...")
